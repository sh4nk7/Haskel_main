...
cleanUp _ [] _ = []
cleanUp (w, h) mat i = map (\(val, index) -> flipValue (val, index) i w) (zip mat [0 ..])
...
printMatrix (w, h) matrix = do
  ...
  putStrLn "\nMatrix:"
  mapM_ (\row -> putStrLn $ unwords $ map (\b -> if b then "O" else "X") row) rows

-- lets the user play the game
playGame :: (Int, Int) -> [Bool] -> Int -> IO ()
